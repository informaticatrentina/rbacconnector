            ...
            foreach ($users as $user) {
              ...
              <tr>
                ...
                <td><?php echo CHtml::link($actionText, array('/rbacconnector/user/assign?id=' . $user['user_id'])); ?> |
                  <?php
                  $statusText = Yii::t('rbac', 'Disabled User');
                  $statusLink='disable';
                  $statusClass='disable_rbac_users';
                  if ($user['status'] == 0) {
                    $statusText = Yii::t('rbac', 'Enabled User');
                    $statusLink='enable';
                    $statusClass='enable_rbac_users';
                  }
                  ?>
                  <?php echo CHtml::link(Yii::t('rbac', $statusText), array('/rbacconnector/user/'.$statusLink.'?id=' . $user['user_id']), array('class' => $statusClass)); ?>
                </td>
              </tr>
            <?php } ?>
          </tbody>
